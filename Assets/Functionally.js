  const siteOnline = true;
    ...
    function renderGameDetails(game) {
      ...
      <main id="main-content" class="flex-grow container mx-auto px-4 py-8 max-w-5xl">
        <article class="bg-indigo-800 rounded-2xl shadow-2xl p-8 flex flex-col gap-10">
          <div class="flex flex-col md:flex-row gap-8">
            <div class="md:w-1/2 relative flex-shrink-0 rounded-2xl overflow-hidden shadow-2xl">
              <img id="main-screenshot" src="${game.screenshots[0]}" alt="Main screenshot of the game ${game.name} showing gameplay or environment" class="w-full object-cover max-h-[420px] rounded-2xl" />
              <div class="flex space-x-3 mt-3 overflow-x-auto scrollbar-thin scrollbar-thumb-indigo-600 scrollbar-track-indigo-900 px-1">
                ${game.screenshots.map((src, i) => `
                  <img src="${src}" alt="Screenshot ${i + 1} of the game ${game.name} showing gameplay or environment" class="w-28 h-20 object-cover rounded-lg cursor-pointer border-2 border-transparent hover:border-indigo-400 transition" tabindex="0" data-index="${i}" />
                `).join('')}
              </div>
            </div>
            <div class="md:w-1/2 text-indigo-200 flex flex-col justify-between">
              <h2 class="text-5xl font-extrabold mb-3 text-indigo-300 font-orbitron select-text">${game.name}</h2>
              <div class="text-indigo-400 font-semibold mb-5 select-text tracking-wide">Developer: ${game.developer}</div>
              <div class="maturity-label-details mb-8 text-indigo-300 italic" aria-label="Maturity rating">${game.maturity}</div>
              <div class="btn-group flex flex-wrap gap-5 items-center">
                ${isPublic ? `
          ...
          <section class="text-indigo-200 prose max-w-full" aria-label="Game description" tabindex="0">
            <h3 class="text-3xl font-bold mb-5 text-indigo-300 select-none">Description</h3>
            ${desc}
          </section>
          <section class="text-indigo-200 max-w-full" aria-label="Game specifications" tabindex="0">
            <h3 class="text-3xl font-bold mb-5 text-indigo-300 select-none">Specifications</h3>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-16 gap-y-6 text-indigo-300 text-lg tracking-wide">
              ${specsHTML}
            </div>
          </section>
        </article>
        ...
        postsHTML += `
          <article tabindex="0" class="post bg-indigo-800 rounded-2xl shadow-2xl p-6 flex flex-col text-center text-indigo-200 focus:outline-none focus:ring-4 focus:ring-indigo-500 transition-transform hover:scale-[1.03]">
            <div class="news-post-image-container mb-6 rounded-2xl overflow-hidden relative">
              <img src="${post.image}" alt="News post titled '${post.title}' image" loading="lazy" class="rounded-2xl" />
    ...
    function renderHome() {
      ...
      <section class="full-width-banner max-w-full bg-gradient-to-r from-indigo-900 via-purple-900 to-indigo-900 text-center text-indigo-100 p-16 rounded-3xl shadow-2xl font-orbitron mx-6 sm:mx-auto max-w-7xl select-none" role="banner" aria-label="Main promotional banner">
        <div>
          <h1 class="text-5xl sm:text-6xl font-extrabold mb-6 leading-tight" style="text-shadow: 0 0 12px #a78bfa;">Unleashing the Potential of Metaverse Gaming</h1>
